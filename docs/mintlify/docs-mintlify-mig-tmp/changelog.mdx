...
- **Windows: installs no longer fail when screenpipe is running** — the Windows installer could fail with an "error opening file for writing" dialog when an orphaned screenpipe process still held one of the app's binaries open, and the existing kill-and-retry fallback couldn't always unblock it. The installer now renames locked files out of the way instead of trying to overwrite them, sweeps the leftovers on next install and at app startup, and shows a clear, actionable message in the rare case a file can't be renamed at all — so updates finish cleanly and Retry actually works. See [troubleshooting](/troubleshooting).
