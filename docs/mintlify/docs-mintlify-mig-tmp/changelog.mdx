...
- **"Pipes" is now "Scheduled"** — the desktop app's Pipes surface has been renamed throughout the UI to make it clearer what it does. The sidebar entry is now **Scheduled**, the page heading is **Scheduled tasks**, the **My Pipes** tab is **My tasks**, and the marketplace is just **Store**. Run history, onboarding, settings copy, Live Views, and toast/error strings all use the new naming. Internal contracts are unchanged: the `/pipes` routes, `pipe.md`, CLI commands, API paths, event names, and any pipes you've already installed keep working exactly as before. See [pipe store](/pipe-store).
