...
- **Overlay recording pill names which subsystem failed** — the capture-health pill in the overlay used to read "recording needs help" no matter what actually broke, so a stalled microphone and a wedged screen backend looked identical. It now reads "audio needs help" when only the mic is affected and "screen capture needs help" when only capture is affected. The generic "recording needs help" only appears when both subsystems (or an unknown one) are involved. Click-to-restart, the urgent-state styling, and pause behavior are unchanged. See [troubleshooting](/troubleshooting).
