...
- **Living commitments inbox in Live Views** — Live View lists can now behave as a durable work queue: any card item can be resolved, snoozed for an hour / until tomorrow / next week, corrected, dismissed, or handed off through chat, with reversible receipts and a **Reopen** action for anything you closed by mistake. Each item keeps a stable identity across refreshes, so state survives new context and rewording. Two ready-made kits ship with it — **Commitments** (promises, requests, waiting items, deadlines) and **Accounting follow-through** (payment/document gaps) — both disabled until you enable them. Handoffs to Slack, Linear, or similar destinations always require an explicit confirmation. See [second brain](/second-brain) and [pipes](/pipes).
- **Timeline opt-out during onboarding on low-end devices** — new users on a confirmed low-tier machine (roughly 8 GB RAM or a small CPU) now see a **Timeline on / keep it off** step before the recorder starts. Choosing **keep it off** disables the visual Timeline, screenshot capture, JPEG writes, OCR fallback, and compaction, while accessibility-tree capture continues so search, ask, and pipes stay fully functional. Every other device follows the existing flow unchanged. Managed deployments honor `disableTimeline` / `disableScreenshots` policies. See [getting started](/getting-started) and [architecture](/architecture).
- **Lifetime users can add the Business plan** — Lifetime customers can now add the Business plan directly from desktop billing without giving up their Lifetime seat. See [teams](/teams).
- **Claude Opus 5 and Sonnet 5 in the model picker** — screenpipe's AI provider list now includes Anthropic Claude Opus 5 and completes Sonnet 5 coverage, so chat and pipes can target either model. See [ai memory](/ai-memory).

### updates

- **Structured app-context modes exposed on the CLI** — the semantic app-context pipeline added last week is now configurable from the CLI in addition to Settings, so scripted and enterprise installs can pick a mode at spawn time. See [cli reference](/cli-reference) and [permissions](/permissions).
- **AI gateway avoids recommending Auto in blocked regions** — when Anthropic returns a geo-restriction error, screenpipe now recognizes it and stops recommending the **Auto** provider in regions where it can't succeed, so you get a clear signal to pick a different provider instead of silent failures. See [ai memory](/ai-memory).
- **Faster AI gateway entitlement checks** — entitlement lookups against the AI gateway are now cached, so chat and pipes start responding sooner after login and don't re-check on every request. See [ai memory](/ai-memory).
- **Simpler Live View action hierarchy** — the actions row on Live View cards has been condensed so the primary action is easier to hit, with secondary actions collected into a single overflow. See [second brain](/second-brain).
- **Notification actions truncate cleanly** — long notification action labels no longer overflow their buttons; extra text is truncated with an ellipsis and stays readable on narrow layouts.
- **Auto-update opt-in is persisted before restart** — toggling auto-updates now saves your choice before the app restarts to apply it, so the setting can't get lost if the restart is interrupted.

### bug fixes

- **False "recording needs help" incidents on macOS** — on a healthy but idle Mac, a wedged ScreenCaptureKit callback could freeze the capture loop, flip `/health` to `degraded` (503), and trigger an incident overlay when nothing was actually being missed. Every ScreenCaptureKit call is now bounded with an explicit timeout, the visual-change probe can no longer freeze the loop, and the incident detector now tiers bare "stale" states by user presence — alerts fire at ~90s only when there's real user activity being lost, and idle stalls wait longer so the engine's own watchdog can self-heal first. Real, sustained wedges still surface as before. Recovery also now requires 90 consecutive healthy checks before the overlay reports green, so a single fresh frame after a watchdog restart no longer misreports the recorder as healthy. See [troubleshooting](/troubleshooting) and [permissions](/permissions).
- **Post-meeting memory growth on macOS and Windows** — audio capture and the meeting editor could hold onto native memory after a meeting ended. Both are now bounded, so long recording sessions with many meetings no longer trend up in RAM. See [meeting intelligence](/meeting-intelligence).
- **Audio devices released between meetings-only sessions** — in meetings-only mode, input devices are now released when a session ends instead of staying held between meetings, so other apps can use the microphone without contention.
- **Post-writer-stall duration in captured meetings** — after a rare recording-writer stall, saved meeting clips could report the wrong duration; wall-clock length is now preserved so seeking and thumbnails line up. See [meeting intelligence](/meeting-intelligence).
- **Chat sidebar stays consistent across windows** — active chat turns are now synchronized across windows, repeated prompts no longer disappear from the sidebar, branched chats stay put, and new turns from Pi never leak into an unrelated previous chat file. Long-running turns can also resume across window reloads. See [ai memory](/ai-memory).
- **Timeline links preserve your timezone** — timeline links produced by chat now carry the original timezone offset, so clicking one from a different time zone lands on the correct moment.
- **Linux AppImage GStreamer plugins restored** — screen and audio capture now find the host's GStreamer plugins from the AppImage on more distros, fixing capture that could fail silently on some Linux setups.
- **Windows local crash dump recovery** — Windows installs now recover fail-fast crash dumps locally so the app can restart cleanly without leaving stale state behind.
- **Enterprise host identity is more robust** — enterprise-managed devices report a more stable host identity to the fleet, reducing duplicate registrations after hostname or hardware changes. See [teams](/teams).
- **Pi prompts start reliably on Windows** — a Windows-only path that could occasionally leave a Pi prompt hung on start now runs its watchdog under coverage, so prompts start without silent hangs. See [ai memory](/ai-memory).
- **AI provider tokens never sent to the wrong host** — the AI gateway will no longer route trusted-runner chat requests to mismatched hosts, and Pi session history is no longer replayed twice, so responses stay consistent.
