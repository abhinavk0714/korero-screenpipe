...
- **Exclude specific apps from system-audio capture (macOS 14.4+)** — keep individual apps out of screenpipe's system-audio recording without turning off audio entirely. Manage the list from **Settings → Recording → Exclude apps from system audio** using a Finder `.app` picker with app icons, or hot-reload a JSON file at `~/.screenpipe/audio-exclusions.json`. The first add triggers macOS's audio-capture permission prompt. See [privacy filter](/privacy-filter) and [permissions](/permissions).
- **Bring your own MCP servers** — register custom MCP servers from your config and route them through the AI agent alongside the built-in ones, so tools you already run locally show up as first-class agent capabilities. See [mcp server](/mcp-server).
- **Cleaner microphone capture on Apple silicon** — microphone capture can now use macOS VoiceProcessingIO (opt-in) for echo and noise cancellation, which helps in meetings and noisy rooms. See [meeting transcription](/meeting-transcription).

### updates

- **Consistent Brain navigation and a smarter Live View composer** — the Home window's Brain navigation is now consistent across Live Views, and the composer for creating a new Live View is collapsible and adapts to how you use it, so the workspace stays out of the way when you're reading and expands when you're building. See [second brain](/second-brain).

### bug fixes

- **Electron apps get the full accessibility walk budget** — VS Code, Discord, Slack, and other Electron apps no longer get their accessibility tree cut off halfway through. The depth counter now resets at `AXWebArea` boundaries so screenpipe reads the whole window. See [troubleshooting](/troubleshooting).
- **USB audio devices with mismatched sample rates no longer drop into silence on macOS** — plugging in a USB mic or interface whose sample rate didn't match the system's could leave the input silent until you unplugged it. Capture now handles the mismatch and keeps recording. See [troubleshooting](/troubleshooting).
