...
describe("Automate My Work evaluations", () => {
  ...
  it("injects the existing inventory and requires one evidence-backed proposal before writes", () => {
    ...
    expect(prompt).toContain("/executions?limit=1");
    expect(prompt).toContain("this proves only that the run started");
  ...
  it("keeps the fresh-install template on the same evidence-first contract", () => {
    ...
    expect(bundledTemplate).toContain("/executions?limit=1");
    expect(bundledTemplate).toContain("this proves only that the run started");
