...
export function buildMeetingSummarizeInstructions(
  ...
  if (options?.followUpAsk) {
    lines.push(
      ``,
      `after the PUT, offer 2-3 useful options from the user's *connected* apps — ask first, never push on your own. don't guess at the integration list: GET http://localhost:3030/connections and keep only the ones with "connected": true. connection availability alone is not evidence that the user prefers that app: put an app first only when the meeting context or a prior exact destination makes it relevant; otherwise order options alphabetically. ask in one short message which option (if any) to use. if nothing is connected, say so in one line (connecting Notion/Slack/Telegram/… would let you push next time) and stop. when they pick one, push via that connection's endpoint (POST /connections/<id>/send for slack/telegram/discord, POST /connections/<id>/proxy/... for notion/linear/etc.) and confirm the exact channel, chat, or parent page before anything leaves the machine.`,
    );
  }
